...
namespace BKYL.BFES.Plugin
{
    ...
    class Job_ConvertMesData : BaseJob
    {
        ...
        List<MES_PUBLIC_MATERIAL> MespublicList = null;
...
"S","ZNO","R2","R3","R4","CAO_2SIO2","MAT_SAMPLE_NO","MAT_SAMPLE_TIME"};
        ...
        private void GetBaseData()
        {
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                HopperModel = idatabase.GetDataTable("select * from MES_TMP_HOPPER where record_time= (select max(record_time) from MES_TMP_HOPPER)");
                MespublicList = idatabase.Queryable<MES_PUBLIC_MATERIAL>().ToList();
                ...
                using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
                {
                    if (model.CODE >= 10 && model.CODE <= 19)
                    {
                        MES_TQMIR21_IOOPList = idatabase.GetList<MES_TQMIR21_IOOP>(
                            ...
                            " and MAT_SAMPLE_NO  like 'UMF%' ", startTime, endTime, model.CODEL3));
                    }
                    else if (model.CODE == 1) //布袋
                    {
                        MES_TQMIR21_IOOPList = idatabase.GetList<MES_TQMIR21_IOOP>(
                            ...
                            "and to_date(REC_REVISE_TIME,'yyyymmddhh24miss')>=to_date('{0}','yyyy/mm/dd hh24:mi:ss')" +
                            " and to_date(REC_REVISE_TIME,'yyyymmddhh24miss')<=to_date('{1}','yyyy/mm/dd hh24:mi:ss')" +
                            " and MAT_SAMPLE_NO  like 'LWH%20' ", startTime, endTime, model.CODEL3));
                    }
                    else if (model.CODE == 2) //重力
                    {
                        MES_TQMIR21_IOOPList = idatabase.GetList<MES_TQMIR21_IOOP>(
                            ...
                            "and to_date(REC_REVISE_TIME,'yyyymmddhh24miss')>=to_date('{0}','yyyy/mm/dd hh24:mi:ss')" +
                            " and to_date(REC_REVISE_TIME,'yyyymmddhh24miss')<=to_date('{1}','yyyy/mm/dd hh24:mi:ss')" +
                            " and MAT_SAMPLE_NO  like 'LWH%10' ", startTime, endTime, model.CODEL3));
                    }
        ...
        private void GetSampleNo(DateTime startTime, DateTime endTime)
        {
            ...
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                sql = string.Format("select * from ( " +
                                    " select * from MES_TQMIR21_IOOP t where mat_sample_no like 'T61%E_1' or mat_sample_no like 'T62%E_1' union" +
                                   " select * from MES_TQMIR21_IOOP t where mat_sample_no like 'T61%F_1' or mat_sample_no like 'T62%F_1' union" +
                                    " select * from MES_TQMIR21_IOOP t where mat_sample_no like 'T61%D_1' or mat_sample_no like 'T62%D_1' union" +
                                    " select * from MES_TQMIR21_IOOP t where mat_sample_no like 'T61%L01' or mat_sample_no like 'T62%L01' )" +
                ...
                sql = string.Format("select * from MES_TQMIR21_IOOP where to_date(REC_REVISE_TIME,'yyyymmddhh24miss')>=to_date('{0}','yyyy/mm/dd hh24:mi:ss') and to_date(REC_REVISE_TIME,'yyyymmddhh24miss')<=to_date('{1}','yyyy/mm/dd hh24:mi:ss')   and MAT_PROD_CODE='C60600100300002A'  and MAT_SAMPLE_NO like 'T6%'", startTime, endTime);
