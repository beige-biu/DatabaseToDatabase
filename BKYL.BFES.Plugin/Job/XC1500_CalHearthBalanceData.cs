...
{
    public class XC1500_CalHearthBalanceData : BaseJob
    {
        List<V_TAPPING> tappingList = new List<V_TAPPING>();
        ...
        public override void RunTask(DateTime currentTime)
        {
            ...
            tappingList = tappingList.GroupBy(x => x.M_TAPNUMB).Select(x => x.OrderBy(y => y.TIMESTAMP).Last()).ToList();
            batchhisList = GetBatchhis(startTime, currentTime);
            if (batchhisList == null)
            {
                tappingList.Clear();
                tappingList = null;
                return;
            ...
            foreach (V_TAPPING item in tappingList)
            {
                timestamp = item.M_TAPSTART.ToDate();
                ...
                {
                    TIMESTAMP = timestamp,
                    TAPNUMB = item.M_TAPNUMB,
                    TAP_START = item.M_TAPSTART.ToDate(),
                    HOLENUMB = item.M_HOLENUMB,
                    TAP_END = item.M_TAPEND.ToDate(),
                    BC = item.BC,
                    BB = item.BB,
                    IRON_REAL = item.M_YIEL_POUD.ToDouble(),
                    SI = item.M_HMSI.ToDouble(),
                    HMTP = item.M_HMTP,
                    SLAG_START = item.M_SLAGSTART.ToDate()
                };
                if (item.M_TAPEND.ToDate() != DateTime.MinValue&& item.M_TAPSTART.ToDate() != DateTime.MinValue)
                {
                    DateTime LastEnd = tappingList.FindAll(m => m.M_TAPEND < item.M_TAPEND).Max(s => s.M_TAPEND).ToDate();
                    if (LastEnd.ToDate() != DateTime.MinValue)
                    {
                        TapModel.IRONPRODRATIO = (item.M_TAPEND.ToDate() - item.M_TAPSTART.ToDate()).TotalMinutes / (item.M_TAPEND.ToDate() - LastEnd.ToDate()).TotalMinutes * 100;
                    }
                }
                if (item.M_TAPEND.ToDate() != DateTime.MinValue && item.M_SLAGSTART.ToDate() != DateTime.MinValue&& item.M_TAPSTART.ToDate() != DateTime.MinValue)
                {
                    TapModel.SLAGPRODRATIO = (item.M_TAPEND.ToDate() - item.M_SLAGSTART.ToDate()).TotalMinutes / (item.M_TAPEND.ToDate() - item.M_TAPSTART.ToDate()).TotalMinutes * 100;
                }
                TapModel.IRON_THEORY = CalTappingTheroyIron(item.M_TAPNUMB);
                ...
                if (TapModel.TAP_SPEED != null)
                    TapModel.TAP_SPEED = Math.Round(TapModel.TAP_SPEED.ToDouble(), 5);
                if (TapModel.IRONPRODRATIO != null)
                    TapModel.IRONPRODRATIO = Math.Round(TapModel.IRONPRODRATIO.ToDouble(), 5);
                if (TapModel.SLAGPRODRATIO != null)
                    TapModel.SLAGPRODRATIO = Math.Round(TapModel.SLAGPRODRATIO.ToDouble(), 5);
                TapModel.IRON_THEORY = Math.Round(TapModel.IRON_THEORY.ToDouble(), 5);
                TapModel.IRON_REAL = Math.Round(TapModel.IRON_REAL.ToDouble(), 5);
                TapModel.IRONDIFF = Math.Round(TapModel.IRONDIFF.ToDouble(), 5);
            ...
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                try
                {
                    if (TapModelList.Count > 0)
                    {
                        ...
                        int iStart = TapModelList.Min(m => m.TAPNUMB.Substring(2, 6).ToInt());
                        int iEnd = TapModelList.Max(m => m.TAPNUMB.Substring(2, 6).ToInt());
        ...
        /// <returns></returns>
        private List<V_TAPPING> GetNewTapping(DateTime startTime, DateTime endTime)
        {
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                var tempData = idatabase.Queryable<V_TAPPING>().Where(match => match.M_TAPEND > startTime && match.M_TAPEND < endTime);
                if (tempData == null || tempData.Count() <= 0)
                {
                    return null;
                }
                return tempData.OrderBy(m => m.M_TAPNUMB, OrderByType.Desc).ToList();
            }
        }
        /// <summary>
        /// 读取铁水Si
        ...
        /// <returns></returns>
        private List<L3TAPPING_IRON_ANA> GetL3TAPPING_IRON_ANA(string tapNumb)
        {
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                return idatabase.Queryable<L3TAPPING_IRON_ANA>().Where(match => match.TAPNUMB == tapNumb).ToList();
            }
        }
        /// <summary>
        ...
        private List<BATCHHIS> GetBatchhis(DateTime startTime, DateTime endTime)
        {
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                var tempData = idatabase.Queryable<BATCHHIS>().Where(match => match.TIMESTAMP > startTime.AddDays(-1) && match.TIMESTAMP <= endTime);
                if (tempData == null || tempData.Count() <= 0)
                {
                    return null;
                }
                return tempData.OrderBy(m => m.TIMESTAMP, OrderByType.Desc).ToList();
            }
        }
        //获取铁次的理论出铁量
        private double CalTappingTheroyIron(string tapNumb)
        {
            ...
            V_TAPPING model = new V_TAPPING();
            V_TAPPING Lastmodel = new V_TAPPING();
            if (tappingList.Count > 0)
            {
                int index = -1;
                index = tappingList.FindIndex(match => match.M_TAPNUMB == tapNumb);
                if (index >= 0)
                {
                    model = tappingList[index];
                }
                index = -1;
                index = tappingList.FindIndex(match => match.M_TAPNUMB == LastTapNumb);
                if (index >= 0)
                {
            ...
            if (Lastmodel != null)
            {
                Result = batchhisList.FindAll(match => match.TIMESTAMP > Lastmodel.M_TAPEND && match.TIMESTAMP <= model.M_TAPEND).Sum(m => m.A_MHTM).ToDouble();
            }
            return Result;
        }
        /// <summary>
        ...
        {
            return string.Format("T7{0}", tapNumb);
        }
        /// <summary>
        /// 根据铁次号获取记录
        /// </summary>
        /// <param name="tapNumb"></param>
        /// <returns></returns>
        private V_TAPPING GetLastTapping(string tapNumb)
        {
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                return idatabase.GetSingle<V_TAPPING>($"select * from V_TAPPING where M_TAPNUMB ='{tapNumb}'");
            }
        }
        /// <summary>
        ...
        private void CalClassDayDiffIron()
        {
            ...
            foreach (var a in TapHearthIronDiff.GroupBy(m => m.TIMESTAMP))
            {
                ...
                foreach (var item in a.GroupBy(m => m.BC))
                {
                    ...
                    modelTemp.YIELD = modelTemp.IRON_THEORY + modelTemp.IRONDIFF; //理论铁量(batchhis中统计的)+铁量差(以铁次统计的)
        ...
        private List<BATCHHIS_TOTAL> CalBatchhisTotal(DateTime strTime)
        {
            ...
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                Value = idatabase.GetDouble($"select sum(A_WINJ_01H) from AV01H_WINJ where timestamp>{DataBaseFactory.ConvertTimeString(StartTime, DataBaseType.Oracle)}" +
                    $" and timestamp<={DataBaseFactory.ConvertTimeString(strTime, DataBaseType.Oracle)}");
            }
            BATCHHIS_TOTAL tempModel = new BATCHHIS_TOTAL() { TIMESTAMP = InsertTime, CODE = 10, WEIGHT = Value };
