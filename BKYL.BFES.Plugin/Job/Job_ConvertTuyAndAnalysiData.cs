...
{
    class Job_ConvertTuyAndAnalysiData : BaseJob
    {
        ...
        public void ConvertTuyData(DateTime currentTime)
        {
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                try
                {
                    DataTable dt = idatabase.GetDataTable("select TUYERE_ID,TUYERE_DIAMETER,TUYERE_LENGTH,TUYERE_CHGTIME from MES_PUBLIC_TUYERE");
                    idatabase.BeginTran();
                    foreach (DataRow OneRow in dt.Rows)
                    {
                        int TuyNo = OneRow["TUYERE_ID"].ToInt();
                        double Diameter = OneRow["TUYERE_DIAMETER"].ToDouble();
                        double Length = OneRow["TUYERE_LENGTH"].ToDouble();
                        DateTime time = OneRow["TUYERE_CHGTIME"].ToDate();
        ...
        public void ConVertL3MaterialData(DateTime currentTime)
        {
            using (IDataBase idatabase = DalFactory.GreateIDataBase(S_DBSource))
            {
                try
                {
                    DataTable dt_S = idatabase.GetDataTable("select MAT_CODE,E_NAME,C_NAME,C_TYPE,DENSITY,MAT_DESC,MES_CODE from MES_PUBLIC_MATERIAL");
                    L_L3MATERIAL.Clear();
