...
namespace BKYL.Sinter.Plugin
{
    ...
    /// 一混质粒参数 yanhao
    /// </summary>
    public class Console_1M : BaseJob
    {      
        ...
        public override void RunTask(DateTime currentTime)
        {
            ...
            try
            {
                using (IDataBase iDataBase = DalFactory.GreateIDataBase(T_DBSource))
                {
                    ...
                    DateTime M_PICAL_1M_RESULT_time = M_PICAL_1M_result(iDataBase);//获取最大时间
                    MD_PHY_PARTICLE_INFO_IN MD_PHY_PARTICLE_INFO_IN = GET_MD_PHY_PARTICLE_INFO_IN(iDataBase);//获取来源表信息                            
                    //如果来源表有新的数据插入
                    if (M_PICAL_1M_RESULT_time!= DateTime.MinValue && MD_PHY_PARTICLE_INFO_IN.TIMESTAMP > M_PICAL_1M_RESULT_time)
                    {
                        T_BLEND_PLC_1MIN T_BLEND_PLC_1MIN = GET_T_BLEND_PLC_1MIN(MC_MICAL_PAR_, M_PICAL_PAR_,iDataBase);//获取GET_T_BLEND_PLC_1MIN平均值
                        MC_MICAL_RESULT MC_MICAL_RESULT_ = GET_MC_MICAL_RESULT(MD_PHY_PARTICLE_INFO_IN,iDataBase);//获取时间1M_MAT_TIME
                        List<MC_SINCAL_INTERFACE_TRIG_MIN> MC_SINCAL_INTERFACE_TRIG_MIN = GET_MC_SINCAL_INTERFACE_TRIG_MIN(MC_MICAL_PAR_, MC_MICAL_RESULT_,M_PICAL_PAR_, iDataBase);//获取MC_SINCAL_INTERFACE_TRIG_MIN平均值(烧返)                       
                        ...
                        SET_SAVE(MD_PHY_PARTICLE_INFO_IN, T_BLEND_PLC_1MIN, MC_SINCAL_INTERFACE_TRIG_MIN, MC_SINCAL_INTERFACE_TRIG_MIN_M, MD_PHY_PARTICLE_INFO_IN_HY);
                    }
                    else
                    {
                        log = "不满足条件";
        ...
        public MC_MICAL_RESULT GET_MC_MICAL_RESULT(MD_PHY_PARTICLE_INFO_IN MD_PHY_PARTICLE_INFO_IN, IDataBase iDataBase)
        {
            ...
            string str_sql = string.Format("select * from MC_MICAL_RESULT where TIMESTAMP=(select max(timestamp)  from MC_MICAL_RESULT where timestamp <= to_date('" + SAMPLTIME + "','yyyy-mm-dd hh24:mi:ss'))and datanum=1");
            return iDataBase.GetSingle<MC_MICAL_RESULT>(str_sql);
        }
        //获取最大时间   
        //public M_PICAL_1M_RESULT M_PICAL_1M_result(IDataBase iDataBase)
        //{
        //    string str_sql = string.Format("select max(TIMESTAMP)  as TIMESTAMP from M_PICAL_1M_result");
        //    return iDataBase.GetSingle<M_PICAL_1M_RESULT>(str_sql);
        //}

        public DateTime M_PICAL_1M_result(IDataBase iDataBase)
        {
            string str_sql = string.Format("select max(TIMESTAMP)  as TIMESTAMP from M_PICAL_1M_result");
            return iDataBase.GetDateTime(str_sql);
        }
        public MD_PHY_PARTICLE_INFO_IN GET_MD_PHY_PARTICLE_INFO_IN(IDataBase iDataBase)
        {
            string str_sql = string.Format("select * from MD_PHY_PARTICLE_INFO_IN where TIMESTAMP=(select max(TIMESTAMP) from MD_PHY_PARTICLE_INFO_IN where 1=1 and L2_CODE =801)");
        ...
        public T_BLEND_PLC_1MIN GET_T_BLEND_PLC_1MIN(MC_MICAL_PAR MC_MICAL_PAR_, M_PICAL_PAR M_PICAL_PAR_,IDataBase iDataBase)
        {
            ...
            double TIME_1X = MC_MICAL_PAR_.PAR_1M_U_TIME.ToDouble();//获取倒推分钟数
            DateTime DTmaxDate = maxDate.AddMinutes(-TIME_1X);//获取倒退最大时间
            double PAR_T1 = M_PICAL_PAR_.PAR_T1.ToDouble();//获取平均时间分钟数
            DateTime DTminDate = DTmaxDate.AddMinutes(-PAR_T1);//获取倒退最小时间
            string str_sql = string.Format("select  round(avg(B_1M_PRE_BELT_W_1H_1),3) B_1M_PRE_BELT_W_1H_1,round(avg(B_1M_TARGET_WATER), 3) B_1M_TARGET_WATER,round(avg(B_1M_FT_PV), 3) B_1M_FT_PV,  round(avg(B_1M_MIXER_RATE), 3) B_1M_MIXER_RATE  from T_BLEND_PLC_1MIN  where timestamp > = to_date('" + DTminDate + "','yyyy-mm-dd hh24:mi:ss')  and  timestamp < to_date('" + DTmaxDate + "','yyyy-mm-dd hh24:mi:ss')");
        ...
        public List<MC_SINCAL_INTERFACE_TRIG_MIN> GET_MC_SINCAL_INTERFACE_TRIG_MIN(MC_MICAL_PAR MC_MICAL_PAR_, MC_MICAL_RESULT MC_MICAL_RESULT_, M_PICAL_PAR M_PICAL_PAR_, IDataBase iDataBase)
        {
            ...
            double TIME_1X = MC_MICAL_PAR_.PAR_1M_U_TIME.ToDouble();//获取倒推分钟数
            double MAT_TIME_1M = MC_MICAL_RESULT_.MICAL_1M_MAT_TIME.ToDouble();//获取倒推分钟数
            double T1 = (TIME_1X + MAT_TIME_1M).ToDouble();
            DateTime DTmaxDate = maxDate.AddMinutes(-T1);//获取倒退最大时间
            double PAR_T1 = M_PICAL_PAR_.PAR_T1.ToDouble();//获取平均时间分钟数
            DateTime DTminDate = DTmaxDate.AddMinutes(-PAR_T1);//获取倒退最小时间
        ...
        public void SET_SAVE(MD_PHY_PARTICLE_INFO_IN MD_PHY_PARTICLE_INFO_IN, T_BLEND_PLC_1MIN T_BLEND_PLC_1MIN, List<MC_SINCAL_INTERFACE_TRIG_MIN> MC_SINCAL_INTERFACE_TRIG_MIN, MC_SINCAL_INTERFACE_TRIG_MIN MC_SINCAL_INTERFACE_TRIG_MIN_M, MD_PHY_PARTICLE_INFO_IN MD_PHY_PARTICLE_INFO_IN_HY)
        {
            string log = "";
            M_PICAL_1M_RESULT M_PICAL_1M_RESULT_M = null;
            M_PICAL_1M_RESULT_M = new M_PICAL_1M_RESULT();

            M_PICAL_1M_RESULT_M.TIMESTAMP = MD_PHY_PARTICLE_INFO_IN.TIMESTAMP.ToDate();
            M_PICAL_1M_RESULT_M.PICAL_1M_TIME = MD_PHY_PARTICLE_INFO_IN.TIMESTAMP.ToDate();
            M_PICAL_1M_RESULT_M.PICAL_1M_GRIT_1 = MD_PHY_PARTICLE_INFO_IN.GRIT_LOW_1.ToDouble();
            M_PICAL_1M_RESULT_M.PICAL_1M_GRIT_1_3 = MD_PHY_PARTICLE_INFO_IN.GRIT_1_3.ToDouble();
            M_PICAL_1M_RESULT_M.PICAL_1M_GRIT_3 = MD_PHY_PARTICLE_INFO_IN.GRIT_UP_3.ToDouble();

            M_PICAL_1M_RESULT_M.PICAL_1M_FILL_RATE = T_BLEND_PLC_1MIN.B_1M_PRE_BELT_W_1H_1.ToDouble();
            M_PICAL_1M_RESULT_M.PICAL_1M_TARGET_WATER = T_BLEND_PLC_1MIN.B_1M_TARGET_WATER.ToDouble();
            M_PICAL_1M_RESULT_M.PICAL_1M_FT_PV = T_BLEND_PLC_1MIN.B_1M_FT_PV.ToDouble();
            M_PICAL_1M_RESULT_M.PICAL_1M_MIXER_RATE = T_BLEND_PLC_1MIN.B_1M_MIXER_RATE.ToDouble();

            if (MC_SINCAL_INTERFACE_TRIG_MIN.Count == 1)
            {
                M_PICAL_1M_RESULT_M.PICAL_1M_SOL_BILL_1 = MC_SINCAL_INTERFACE_TRIG_MIN[0].SINCAL_ORE_SP.ToDouble();
            }
            if (MC_SINCAL_INTERFACE_TRIG_MIN.Count == 2)
            {
                M_PICAL_1M_RESULT_M.PICAL_1M_SOL_BILL_1 = MC_SINCAL_INTERFACE_TRIG_MIN[0].SINCAL_ORE_SP.ToDouble();
                M_PICAL_1M_RESULT_M.PICAL_1M_SOL_BILL_2 = MC_SINCAL_INTERFACE_TRIG_MIN[1].SINCAL_ORE_SP.ToDouble();
            }
            if (MC_SINCAL_INTERFACE_TRIG_MIN.Count >= 3)
            {
                M_PICAL_1M_RESULT_M.PICAL_1M_SOL_BILL_1 = MC_SINCAL_INTERFACE_TRIG_MIN[0].SINCAL_ORE_SP.ToDouble();
                M_PICAL_1M_RESULT_M.PICAL_1M_SOL_BILL_2 = MC_SINCAL_INTERFACE_TRIG_MIN[1].SINCAL_ORE_SP.ToDouble();
                M_PICAL_1M_RESULT_M.PICAL_1M_SOL_BILL_3 = MC_SINCAL_INTERFACE_TRIG_MIN[2].SINCAL_ORE_SP.ToDouble();
            }
            M_PICAL_1M_RESULT_M.PICAL_RE_BILL_DRY = MC_SINCAL_INTERFACE_TRIG_MIN_M.SINCAL_ORE_SP.ToDouble();
            M_PICAL_1M_RESULT_M.PICAL_1M_IRON_GRIT = MD_PHY_PARTICLE_INFO_IN_HY.GRIT_UP_3.ToDouble();           
            if (Set_Admin_add(M_PICAL_1M_RESULT_M))
            {
                log = "保存完成";
            }
            else
        ...
        public bool Set_Admin_add(M_PICAL_1M_RESULT model)
        {
            bool listmodel = true;
